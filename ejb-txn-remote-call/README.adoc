...
helm install client -f charts/client.yaml wildfly/wildfly --wait --timeout="5m"
helm install server -f charts/server.yaml wildfly/wildfly --wait --timeout="5m"
----
endif::[]
...
To remove the application you need to remove the `WildFlyServer` definitions
