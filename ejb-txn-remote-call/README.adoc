...
this configuration, the script `${PATH_TO_QUICKSTART_DIR}/ejb-txn-remote-call/client/extensions/remote-configuration.cli`
will be executed on `server1`.

[NOTE]
====
`remote-configuration.cli` is configured with properties in `cli.local.properties` and runs against `standalone.xml`
====
...
----
cd ${jbossHomeName}_1;\
./bin/jboss-cli.sh -DremoteServerUsername="quickstartUser" -DremoteServerPassword="quickstartPwd1!" -DpostgresqlUsername="test" -DpostgresqlPassword="test"\
...
Running `remote-configuration.cli` results in the creation of:
...
Then, the PostgreSQL driver needs to be installed on all {productName} servers.
